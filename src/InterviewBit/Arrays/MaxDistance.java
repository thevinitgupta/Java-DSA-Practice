...
public class MaxDistance {
    static class ArrayComparator implements Comparator<Pair> {

        @Override
        public int compare(Pair o1, Pair o2) {
            return o1.ind < o2.ind ? 1 : -1;
        }
    }
    public int maximumGap(final List<Integer> A) {
        int max = Integer.MIN_VALUE, n = A.size();
        PriorityQueue<Pair> pq = new PriorityQueue<>(new ArrayComparator());
        pq.add(new Pair(A.get(n-1),n-1));
        for(int i=n-2;i>=0;i--){

            for(Pair p : pq){
                if(p.ind>i && p.el>=A.get(i)){
                    max = Math.max(p.ind-i,max);
                    break;
                }
            }
            System.out.println(A.get(i)+","+i+"->"+pq.add(new Pair(i,A.get(i))));
        }
